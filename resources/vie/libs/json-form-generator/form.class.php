...
class Form
{
    ...
    public function input($data)
    {
        ...
        if (isset($data['title'])) $tag .= ' ' . $data['title'] . ': <br>';
        $tag .= '
        ...
        $tag .= '<input aria-label="Label" name="' . $data['name'] . '" ';
        ...
        else $tag .= ' class="mdc-text-field__input"';
        $tag .= " />
        </label>
        </p>";
    ...
    public function captcha($data)
    {
        ...
        if (isset($data['title'])) $tag .= ' ' . $data['title'] . ': ';
        if (isset($data['img'])) $tag .= '<img src="'. $data['img'] .'" width="'.(!empty($data['width']) ? $data['width'] : 120).'"> <br>';
        $tag .= '
        <label class="mdc-text-field mdc-text-field--outlined mdc-text-field--textarea mdc-text-field--no-label">
        <span class="mdc-notched-outline">
            <span class="mdc-notched-outline__leading"></span>
    ...
    {
        $tag = '<br><label for="' . $data['name'] . '">' . $data['title'] . "</label><br>";
        $tag .= '<select style="padding: 15px; min-width: 35px" name="' . $data['name'] . '" ';
        if (isset($data['class'])) $tag .= ' class="' . $data['class'] . '"';
        if (isset($data['id'])) $tag .= ' id="' . $data['id'] . '"';
        if (isset($data['onClick'])) $tag .= ' onclick="' . $data['onClick'] . '"';
        if (isset($data['onChange'])) $tag .= ' onchange="' . $data['onChange'] . '"';
    ...
    public function textarea($data)
    {
        ...
        $tag .= '<span class="mdc-text-field__resizer">';
        $tag .= '<textarea  class="mdc-text-field__input" rows="8" cols="40" aria-label="Label" name="' . $data['name'] . '" ';
        if (isset($data['onClick'])) $tag .= ' onclick="' . $data['onClick'] . '"';
        if (isset($data['onChange'])) $tag .= ' onchange="' . $data['onChange'] . '"';
        if (isset($data['required'])) $tag .= ' required';
        if (isset($data['disabled'])) $tag .= ' disabled';
    ...
    public function render()
    {
        ...
        if (isset($data['enctype'])) {
            $this->html .= ' enctype="' . $data['enctype'] . '"';
        }
        if (isset($data['target'])) {
            $this->html .= ' target="' . $data['target'] . '"';
        }
        if (isset($data['autocomplete'])) {
            $this->html .= ' autocomplete="' . $data['autocomplete'] . '"';
        }
